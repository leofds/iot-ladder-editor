...
public class TimerOnDelay extends TimerInstruction {
	...
	public List<Quadruple> generateIR(GenContext context) {
		...
		Symbol value = symbolTable
				.add(new Symbol(getMemory().getName(), Kind.VARIABLE, getMemory().getType(), GenContext.GLOBAL_SCOPE)); // insere
																														// o
																														// timer
																														// na
																														// tabela
		...
		quadruples.add(QuadrupleFactory.createAssignment(doneVar, status));
		return quadruples;
	}

	@Override
	public String toString() {
		return "instruction=TMR ON, " + super.toString();
	}
}
