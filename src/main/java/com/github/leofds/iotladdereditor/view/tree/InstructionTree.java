...
public class InstructionTree extends JTree implements DragGestureListener, DragSourceMotionListener, DragSourceListener{

	private static final long serialVersionUID = 1L;
	private DragSource dragSource;

	public InstructionTree(TreeModel newModel) {
		super(newModel);
	...
	public void dragGestureRecognized(DragGestureEvent dge) {
		...
		try {
			...
			Object obj = customTreeNode.getClazz().newInstance();
			if(obj instanceof LadderInstruction){
				LadderInstruction instruction = (LadderInstruction) obj;			
			...
			}else if(obj instanceof Rung){
				RungView rung = new RungView();
				dge.startDrag(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR),new CollageTransferHandler(rung),this);
				glassPane.setRung(rung);
			}else{
