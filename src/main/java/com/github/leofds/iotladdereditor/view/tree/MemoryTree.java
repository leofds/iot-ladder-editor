...
public class MemoryTree extends JTree implements DragGestureListener, DragSourceMotionListener, DragSourceListener{

	private static final long serialVersionUID = 1L;
	private DragSource dragSource;
	
	public MemoryTree(TreeModel newModel) {
		super(newModel);
	...
	public void dragGestureRecognized(DragGestureEvent dge) {
		...
		try {
			...
			if(obj != null && obj instanceof DeviceMemory){
				DeviceMemory memory = (DeviceMemory) obj;
				dge.startDrag(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR),new CollageTransferHandler(memory),this);
				glassPane.setMemoryItem(memory);
			}else{
